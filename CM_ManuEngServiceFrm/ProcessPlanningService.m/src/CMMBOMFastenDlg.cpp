//===================================================================
// COPYRIGHT  2021/02/04
//===================================================================
// CMMBOMFastenDlg.cpp
// Header definition of class CMMBOMFastenDlg
//===================================================================
//
// Usage notes:
//
//===================================================================
//  2021/02/04 Creation: Code generated by the 3DS wizard
//===================================================================

#include "CMMBOMFastenDlg.h"
#include "CATDlgGridConstraints.h"
#include "CATDlgLabel.h"
#include "CATDlgLabel.h"
#include "CATDlgLabel.h"
#include "CATDlgEditor.h"
#include "CATDlgLabel.h"
#include "CATDlgLabel.h"
#include "CATDlgCombo.h"
#include "CATDlgEditor.h"
#include "CATDlgEditor.h"
#include "CATDlgEditor.h"
#include "CATDlgCheckButton.h"

// Begin of User Code

// End of User Code

CMMBOMFastenDlg::CMMBOMFastenDlg(CATDialog* iParent, const CATString& iName)
	:CATDlgDialog(iParent, iName, 0
		| CATDlgWndOK
		| CATDlgWndCANCEL
		| CATDlgGridLayout
	)
{

	_Label1 = NULL;
	_Label2 = NULL;
	_Label3 = NULL;
	_Editor4 = NULL;
	_Label4 = NULL;
	_Label5 = NULL;
	_Label6 = NULL;
	_Combo1 = NULL;
	_Editor1 = NULL;
	_Editor2 = NULL;
	_Editor3 = NULL;
	_CheckButton1 = NULL;

	// Begin of User Code

	// End of User Code

}

CMMBOMFastenDlg::~CMMBOMFastenDlg()
{

	// Begin of User Code

	// End of User Code
	_Label1 = NULL;
	_Label2 = NULL;
	_Label3 = NULL;
	_Editor4 = NULL;
	_Label4 = NULL;
	_Label5 = NULL;
	_Label6 = NULL;
	_Combo1 = NULL;
	_Editor1 = NULL;
	_Editor2 = NULL;
	_Editor3 = NULL;
	_CheckButton1 = NULL;

}



void CMMBOMFastenDlg::Build()
{
	int style = 0;

	// Begin of User Code

	// End of User Code
	_Label1 = new CATDlgLabel(this, (const char*)"Label1", 0);
	_Label2 = new CATDlgLabel(this, (const char*)"Label2", 0);
	_Label3 = new CATDlgLabel(this, (const char*)"Label3", 0);
	_Editor4 = new CATDlgEditor(this, (const char*)"Editor4", 0);
	_Label4 = new CATDlgLabel(this, (const char*)"Label4", 0);
	_Label5 = new CATDlgLabel(this, (const char*)"Label5", 0);
	_Label6 = new CATDlgLabel(this, (const char*)"Label6", 0);
	_Combo1 = new CATDlgCombo(this, (const char*)"Combo1", 0 | CATDlgCmbDropDown);
	_Editor1 = new CATDlgEditor(this, (const char*)"Editor1", 0);
	_Editor2 = new CATDlgEditor(this, (const char*)"Editor2", 0);
	_Editor3 = new CATDlgEditor(this, (const char*)"Editor3", 0);
	_CheckButton1 = new CATDlgCheckButton(this, (const char*)"CheckButton1", 0);
	_Label1->SetGridConstraints(0, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Label2->SetGridConstraints(1, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Label3->SetGridConstraints(2, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Label4->SetGridConstraints(3, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Label5->SetGridConstraints(4, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Label6->SetGridConstraints(5, 0, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	
	_Editor1->SetSensitivity(CATDlgDisable);
	_Editor1->SetGridConstraints(0, 1, 4, 1, 0 | CATGRID_LEFT | CATGRID_RIGHT | CATGRID_TOP);
	_Editor2->SetSensitivity(CATDlgDisable);
	_Editor2->SetGridConstraints(1, 1, 4, 1, 0 | CATGRID_LEFT | CATGRID_RIGHT | CATGRID_TOP);
	_CheckButton1->SetGridConstraints(2, 1, 1, 1, 0 | CATGRID_LEFT | CATGRID_TOP);
	_Combo1->SetGridConstraints(3, 1, 4, 1, 0 | CATGRID_LEFT | CATGRID_RIGHT | CATGRID_TOP);
	_Editor3->SetSensitivity(CATDlgDisable);
	_Editor3->SetGridConstraints(4, 1, 1, 1, 0 | CATGRID_LEFT | CATGRID_RIGHT | CATGRID_TOP);
	_Editor4->SetGridConstraints(5, 1, 1, 1, 0 | CATGRID_LEFT | CATGRID_RIGHT | CATGRID_TOP);
	

	// Begin of User Code

	// End of User Code

}

CATDlgEditor* CMMBOMFastenDlg::GetEditor(int iCtrlID)
{
	CATDlgEditor* pEditor = NULL;

	switch (iCtrlID)
	{
	case ID_SelectRModelEditor:
		pEditor = _Editor1;
		break;
	case ID_SelectManuAssemEditor:
		pEditor = _Editor2;
		break;
	case ID_FastenNumberEditor:
		pEditor = _Editor3;
		break;
	case ID_FastenUsingEditor:
		pEditor = _Editor4;
		break;
	default:
		break;
	}

	return pEditor;
}

CATDlgCombo* CMMBOMFastenDlg::GetCombo(int iCtrlID)
{
	CATDlgCombo * pCombo = NULL;
	switch (iCtrlID)
	{
	case ID_FastenDetailCombo:
		pCombo = _Combo1;
		break;
	default:
		break;
	}
	return pCombo;
}
CATDlgCheckButton* CMMBOMFastenDlg::GetCheckButton(int iCtrlID)
{
	CATDlgCheckButton * pCheck = NULL;
	switch (iCtrlID)
	{
	case ID_FastenPairCheckButton:
		pCheck = _CheckButton1;
		break;
	default:
		break;
	}
	return pCheck;
}
